...
const ban = (() => {
  ...
  return function ban(_x, _x2, _x3) {
...
const splitTime = (() => {
  ...
  return function splitTime(_x4, _x5) {
    return _ref2.apply(this, arguments);
  };
})();

const getFlowFromSplitTime = (() => {
  var _ref3 = _asyncToGenerator(function* (serverId, accountId, start, end) {
    const time = yield splitTime(start, end);
    const sum = [];
    const getFlow = function getFlow(tableName, startTime, endTime) {
      return knex(tableName).sum('flow as sumFlow').groupBy('id').select(['id']).where({
        id: serverId,
        accountId
      }).whereBetween('time', [startTime, endTime - 1]).then(function (success) {
        if (success[0]) {
    ...
    const result = (yield Promise.all(sum)).reduce(function (a, b) {
      return a + b;
    });
    return result;
  });

  return function getFlowFromSplitTime(_x6, _x7, _x8, _x9) {
...
const check = (() => {
  var _ref4 = _asyncToGenerator(function* (opt) {
    ...
    const serverId = opt.serverId,
          accountId = opt.accountId,
          time = opt.time,
          flow = opt.flow,
          banTime = opt.banTime;

    const accountFlowData = yield knex('account_flow').where({
      serverId, accountId
    }).then(function (s) {
      return s[0];
    });
    ...
    const myFlow = yield getFlowFromSplitTime(serverId, accountId, checkTime, Date.now());
    if (myFlow >= flow) {
      yield ban(serverId, accountId, banTime);
    }
  });

  return function check(_x10) {
