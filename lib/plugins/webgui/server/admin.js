...
exports.getAccountByPort = (() => {
  var _ref = _asyncToGenerator(function* (req, res) {
    try {
      const port = +req.params.port;
      const accountInfo = yield account.getAccount({ port }).then(function (s) {
        return s[0];
      });
      if (!accountInfo) {
...
exports.getOneAccount = (() => {
  var _ref2 = _asyncToGenerator(function* (req, res) {
    try {
      ...
      const accountInfo = yield account.getAccount({ id: accountId }).then(function (s) {
        return s[0];
      });
      if (!accountInfo) {
        return Promise.reject('account not found');
...
exports.getUserRefOrders = (req, res) => {
  const userId = +req.params.userId;
  refOrder.getUserOrders(userId).then(success => {
    res.send(success);
  }).catch(err => {
    console.log(err);
    res.status(403).end();
...
exports.getCsvOrders = (() => {
  var _ref3 = _asyncToGenerator(function* (req, res) {
    ...
    options.filter = req.query.filter || '';
    alipay.getCsvOrder(options).then(function (success) {
      res.setHeader('Content-disposition', 'attachment; filename=download.csv');
      res.setHeader('Content-type', 'text/csv');
      res.send(success.map(function (m) {
...
exports.getRefOrders = (req, res) => {
  ...
  options.filter = req.query.filter || '';
  refOrder.orderListAndPaging(options).then(success => {
    res.send(success);
  }).catch(err => {
    console.log(err);
    res.status(403).end();
  ...
  var _ref5 = _asyncToGenerator(function* (req, res) {
    try {
      ...
      if (port.random) {
        ...
        newPort = yield checkIfPortExists(myPort);
      } else {
        newPort = yield knex('account_plugin').select().whereBetween('port', [port.start, port.end]).orderBy('port', 'ASC').then(function (success) {
  ...
  var _ref6 = _asyncToGenerator(function* (req, res) {
    try {
      ...
      for (let i = 0; i < number; i++) {
        yield refUser.addRefCode(userId, max);
      }
      res.send('success');
    } catch (err) {
      console.log(err);
...
exports.deleteRefCode = (() => {
  var _ref7 = _asyncToGenerator(function* (req, res) {
    try {
      const code = req.params.code;
      yield refUser.deleteRefCode(code);
      res.send('success');
    } catch (err) {
      console.log(err);
      res.status(403).end();
