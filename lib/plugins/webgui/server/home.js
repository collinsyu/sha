        ...
        const getNewPort = () => {
          return knex('webguiSetting').select().where({
            ...
            if (port.random) {
              ...
              //   return port.start;
              // });
              return knex('account_plugin').select().whereBetween('port', [port.start, port.end]).orderBy('port', 'ASC').then(success => {
...
exports.status = (() => {
  var _ref = _asyncToGenerator(function* (req, res) {
    ...
    try {
      const base = yield knex('webguiSetting').select().where({
        key: 'base'
      ...
      const filterColor = colors.filter(function (f) {
        return f.value === base.themePrimary;
      });
      ...
      let multiAccount;
      if (status) {
        email = (yield knex('user').select(['email']).where({ id }).then(function (s) {
          return s[0];
        })).email;
        ...
        refCode = (yield knex('webguiSetting').select().where({
          key: 'webgui_ref'
        }).then(function (success) {
          success[0].value = JSON.parse(success[0].value);
          return success[0].value;
        })).useRef;
        subscribe = (yield knex('webguiSetting').select().where({
          key: 'account'
        }).then(function (success) {
          success[0].value = JSON.parse(success[0].value);
          return success[0].value;
      ...
      if (status === 'normal') {
        ...
        const groupId = (yield knex('user').select(['group']).where({ id }).then(function (s) {
          return s[0];
        })).group;
        multiAccount = (yield knex('group').where({ id: groupId }).then(function (s) {
          return s[0];
        })).multiAccount;
      }
      res.send({
