...
exports.searchSourceUser = (() => {
  var _ref = _asyncToGenerator(function* (req, res) {
    try {
      const search = req.body.search;
      const users = yield knex('user').select(['id', 'username']).where('username', 'like', `%${search}%`).andWhere('id', '>', 1);
      res.send(users);
    } catch (err) {
      console.log(err);
      res.status(403).end();
...
exports.searchRefUser = (() => {
  var _ref2 = _asyncToGenerator(function* (req, res) {
    try {
      const search = req.body.search;
      const users = yield knex('user').select(['user.id as id', 'user.username as username']).leftJoin('webgui_ref', 'webgui_ref.userId', 'user.id').where('user.username', 'like', `%${search}%`).where({ 'user.type': 'normal' }).whereNull('webgui_ref.id');
      res.send(users);
    } catch (err) {
      console.log(err);
      res.status(403).end();
