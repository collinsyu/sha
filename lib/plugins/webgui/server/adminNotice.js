...
exports.getNotice = (req, res) => {
  knex('notice').select(['notice.id as id', 'notice.title as title', 'notice.content as content', 'notice.time as time', 'group.name as groupName']).orderBy('time', 'desc').leftJoin('group', 'notice.group', 'group.id').then(success => {
    success.forEach(f => {
      if (!f.groupName) {
        f.groupName = '所有组';
      }
