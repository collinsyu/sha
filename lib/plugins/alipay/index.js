...
if (config.plugins.alipay && config.plugins.alipay.use) {
  try {
    ...
    config.plugins.alipay.merchantPrivateKey = privateKey.replace(/-----BEGIN RSA PRIVATE KEY-----/, '').replace(/-----END RSA PRIVATE KEY-----/, '').replace(/\n/g, '');
  } catch (err) {}
  try {
    const publicKey = fs.readFileSync(config.plugins.alipay.alipayPublicKey, 'utf8').toString();
    config.plugins.alipay.alipayPublicKey = publicKey.replace(/-----BEGIN PUBLIC KEY-----/, '').replace(/-----END PUBLIC KEY-----/, '').replace(/\n/g, '');
  } catch (err) {}
  ...
  console.log("alipay_f2f", alipay_f2f);
}
...
const createOrder = (() => {
  var _ref = _asyncToGenerator(function* (user, account, orderId) {
    const oldOrder = yield knex('alipay').where({
      user,
...
const orderListAndPaging = (() => {
  var _ref6 = _asyncToGenerator(function* (options = {}) {
    ...
    let orders = knex('alipay').select(['alipay.orderId', 'alipay.orderType', 'webgui_order.name as orderName', 'user.id as userId', 'user.group as group', 'user.username', 'account_plugin.port', 'alipay.amount', 'alipay.status', 'alipay.alipayData', 'alipay.createTime', 'alipay.expireTime']).leftJoin('user', 'user.id', 'alipay.user').leftJoin('account_plugin', 'account_plugin.id', 'alipay.account').leftJoin('webgui_order', 'webgui_order.id', 'alipay.orderType').whereBetween('alipay.createTime', [start, end]);
...
const getCsvOrder = (() => {
  var _ref7 = _asyncToGenerator(function* (options = {}) {
    ...
    let orders = knex('alipay').select(['alipay.orderId', 'alipay.orderType', 'user.id as userId', 'user.group as group', 'user.username', 'account_plugin.port', 'alipay.amount', 'alipay.status', 'alipay.alipayData', 'alipay.createTime', 'alipay.expireTime']).leftJoin('user', 'user.id', 'alipay.user').leftJoin('account_plugin', 'account_plugin.id', 'alipay.account').whereBetween('alipay.createTime', [start, end]);
