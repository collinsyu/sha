...
const newOrder = (() => {
  var _ref = _asyncToGenerator(function* (data) {
    yield knex('webgui_ref_time').insert({
      orderId: newOrderId(),
      user: data.user,
...
const getUserOrders = (() => {
  var _ref2 = _asyncToGenerator(function* (userId) {
    const orders = yield knex('webgui_ref_time').select(['webgui_ref_time.orderId', 'user.id as userId', 'user.group as group', 'user.username', 'account_plugin.port', 'webgui_ref_time.refTime', 'webgui_ref_time.status', 'webgui_ref_time.createTime']).leftJoin('user', 'user.id', 'webgui_ref_time.user').leftJoin('account_plugin', 'account_plugin.id', 'webgui_ref_time.account').where({ 'user.id': userId });
    return orders;
  });
...
const orderListAndPaging = (() => {
  var _ref3 = _asyncToGenerator(function* (options = {}) {
    ...
    let orders = knex('webgui_ref_time').select(['webgui_ref_time.orderId', 'user.id as userId', 'user.group as group', 'user.username', 'account_plugin.port', 'webgui_ref_time.refTime', 'webgui_ref_time.status', 'webgui_ref_time.createTime']).leftJoin('user', 'user.id', 'webgui_ref_time.user').leftJoin('account_plugin', 'account_plugin.id', 'webgui_ref_time.account').whereBetween('webgui_ref_time.createTime', [start, end]);
    ...
    orders = yield orders.orderBy(sort.split('_').slice(0, sort.split('_').length - 1).join('_'), sort.split('_')[sort.split('_').length - 1]).limit(pageSize).offset((page - 1) * pageSize);
