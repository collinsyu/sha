...
const isLogin = message => {
  ...
  if (message.message.text.trim() !== 'login') {
    return false;
  }
  return true;
};
...
const isGetAccount = message => {
  ...
  if (message.message.text.trim() !== 'account') {
    return false;
  }
  return true;
};
...
const isCallbackAccount = message => {
  if (!message.callback_query || !message.callback_query.data) {
    return false;
  }
...
telegram.on('message', (() => {
  var _ref = _asyncToGenerator(function* (message) {
    if (isGetAccount(message)) {
      ...
      const userId = yield isUser(telegramId);
      const myAccount = yield account.getAccount({ userId });
      if (!myAccount.length) {
      ...
      const keyboard = myAccount.map(function (m) {
        return {
      ...
      const userId = yield isUser(telegramId);
      const myAccount = (yield account.getAccount({ userId, id: +accountId }))[0];
      if (!myAccount) {
        return;
      }
      let servers = yield server.list();
      const validServers = JSON.parse(myAccount.server);
      servers = servers.filter(function (f) {
        if (!validServers) {
          return true;
        } else {
      ...
      if (serverArray.length > 1 && serverArray[serverArray.length - 1].length < row) {
        '----'.substr(0, row - serverArray[serverArray.length - 1].length).split('').forEach(function (f) {
          serverArray[serverArray.length - 1].push({
            text: f,
            callback_data: ' '
          });
      ...
      const userId = yield isUser(telegramId);
      const myAccount = (yield account.getAccount({ userId, id: +accountId }))[0];
      const myServer = (yield server.list()).filter(function (server) {
      ...
      if (myAccount.type >= 2 && myAccount.type <= 5) {
        ...
        const currentFlow = (yield flow.getServerPortFlowWithScale(myServer.id, myAccount.id, timeArray, myAccount.multiServerFlow))[0];
      ...
      const groupInfo = yield knex('group').select(['group.order as order']).leftJoin('user', 'user.group', 'group.id').leftJoin('account_plugin', 'account_plugin.userId', 'user.id').where({ 'account_plugin.id': accountId });
      const groupOrderInfo = groupInfo[0].order ? JSON.parse(groupInfo[0].order) : null;
      let orders = yield orderPlugin.getOrders();
      orders = groupOrderInfo ? orders.filter(function (order) {
      ...
      //   f.id === +orderId;
      // })[0].alipay;
      const userId = (yield tg.getUserStatus(telegramId)).id;
      const payInfo = yield alipay.createOrder(userId, accountId > 0 ? accountId : null, +orderId);
      const qrcodeId = crypto.randomBytes(32).toString('hex');
