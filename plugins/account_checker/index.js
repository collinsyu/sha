...
const isPortExists = async (server, account,type) => {
  const ports = (await manager.send({ command: 'list',shit:"fuck",type}, {
    host: server.host,
    port: server.port,
...
const deleteExtraPorts = async serverInfo => {
  try {
    const currentPorts = await manager.send({ command: 'list',shit:"fuck me" }, {
...
const checkAccount = async (serverId, accountId,type) => {
  try {
    ...
    // 检查当前端口是否存在
    const exists = await isPortExists(serverInfo, accountInfo,type);
  ...
  while(true) {
    ...
    if(accounts.length <= 120) {
      for(const account of accounts) {
        const start = Date.now();
        // 我也不知道这个功能是干啥的，反正改成了30s一次吧！
        // await sleep(20 * 1000);
        await checkAccount(account.serverId, account.accountId,"no promise").catch();
      ...
      await Promise.all(accounts.map((account, index) => {
        return sleep(index * (60 + Math.ceil(accounts.length % 10)) * 1000 / accounts.length).then(() => {
          return checkAccount(account.serverId, account.accountId,"in promise");
        });
      }));
    }
    if(accounts.length) {
